...
def test_user_short_name(base_user):
    ...
    assert base_user.get_short_name() == f"{base_user.first_name} {base_user.last_name}"


def test_user_full_name(base_user):
    """Test user model's get_full_name method"""
    assert base_user.get_full_name == f"{base_user.first_name} {base_user.last_name} {base_user.fathers_name}" 
...
def test_create_user_with_no_fathersname(user_factory):
    """Test creating new user without a fathers name gives an error"""
    with pytest.raises(ValueError) as err:
        user_factory.create(fathers_name=None)
    assert str(err.value) == "Users must submit a fathers name"    
...
def test_user_email_incorrect(user_factory):
    ...
    with pytest.raises(ValueError) as err:
        user_factory.create(email="learning-union.com")
